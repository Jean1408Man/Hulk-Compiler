...
namespace Hulk {
    ...
    void TypeChecker::check_conforms(Expr* node, const HulkType& expected, const std::string& context) {
        ...
        if (!type_conforms_with_inference(found, expected)) {
            report_error(node->span, "Error de tipos en " + context + ": se esperaba '" + 
                         expected.to_string() + "' pero se encontró '" + found.to_string() + "'.");
        }
    }

    bool TypeChecker::type_conforms_with_inference(const HulkType& found,
                                                   const HulkType& expected) const {
    ...
    bool TypeChecker::type_conforms_to_protocol_inferred(
    ...
    bool TypeChecker::method_satisfies_protocol_inferred(
    ...
    HulkType TypeChecker::resolve_method_param_type(const SemanticMethodInfo& method,
    ...
    HulkType TypeChecker::resolve_method_return_type(const SemanticMethodInfo& method) const {
    ...
    HulkType TypeChecker::from_string_type(const std::string& name) const {
